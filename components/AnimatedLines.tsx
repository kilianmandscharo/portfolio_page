...
const AnimatedLines = ({
    ...
    const { greenAnimatedOffset } = useSpring({
        reset: true,
        from: { greenAnimatedOffset: height },
        to: { greenAnimatedOffset: height - 179 },
        delay: 200,
        config: {
            duration: 1000,
        },
    });
    const { purpleAnimatedOffset } = useSpring({
        reset: true,
        from: { purpleAnimatedOffset: height },
        to: { purpleAnimatedOffset: height - purpleOffset - 70 },
        delay: 200,
        config: {
            duration: 1000,
        },
    });
    const { blueAnimatedOffset } = useSpring({
        reset: true,
        from: { blueAnimatedOffset: height },
        to: { blueAnimatedOffset: height - blueOffset - 73 },
        delay: 200,
        config: {
            duration: 1000,
        },
    ...
    return (
        ...
        >
            <animated.path
                ...
                strokeDashoffset={
                    pathsAnimated
                        ? height - 179 - scrollY - extraOffset
                        : greenAnimatedOffset
                }
            />
            <animated.path
